...
# Valid salary expectation labels from the Tally dropdown
SALARY_OPTIONS = [
    "Under $50,000",
    "$50,000 - $75,000",
    "$75,000 - $100,000",
...
# Map lowercase label → canonical label for safe matching
SALARY_LABEL_MAP = {s.lower(): s for s in SALARY_OPTIONS}


def resolve_salary(value: str) -> str:
    """Return the canonical salary label if matched, otherwise return value as-is."""
    return SALARY_LABEL_MAP.get(value.lower().strip(), value)
...
def parse_tally_lead(payload: dict) -> dict:
    ...
    for field in data.get("fields", []):
        ...
        if isinstance(value, list):
            # Tally sends dropdown/multiple choice as a list of dicts: [{"id": "...", "text": "..."}]
            # or sometimes as a plain list of strings
            labels = []
            for item in value:
                if isinstance(item, dict):
                    # Prefer "text" label over raw "id"
                    labels.append(item.get("text") or item.get("label") or str(item))
                else:
